...
\par Cette annexe présente les captures d'écran des configurations de pare-feu déployées dans l'infrastructure. Elle couvre les règles OPNsense inter-zones, les IPSets et groupes de sécurité Proxmox, ainsi que les règles par VM. Ces captures permettent de vérifier visuellement la correspondance entre les tableaux du chapitre IV et les paramètres effectivement appliqués dans les interfaces de gestion.
...
\par La Figure~\ref{fig:ann-fw-opnsense-rules} présente les règles de filtrage configurées dans OPNsense pour chacune des trois zones applicatives. On y retrouve les règles ALLOW pour les flux autorisés (Dev vers WAN, DMZ vers WAN, DMZ vers Prod port 8000) et les règles BLOCK pour les flux interdits (Dev vers Prod, Dev vers DMZ, Prod vers WAN), conformément au tableau~\ref{tab:fw-opnsense-rules} du chapitre IV.\\
...
\par La Figure~\ref{fig:ann-fw-proxmox-ipsets} présente les cinq IPSets conservés après la suppression du doublon \texttt{vxlan-phys-net}, conformément au tableau~\ref{tab:fw-datacenter-ipsets} du chapitre IV. Chaque IPSet regroupe les adresses d'un réseau logique distinct et est référencé par les groupes de sécurité pour éviter la duplication des règles.\\
...
\par La Figure~\ref{fig:ann-fw-proxmox-secgroups} présente les groupes de sécurité après révision. Le groupe \texttt{ceph-storage} affiche désormais quatre entrées : deux pour le réseau interne (\texttt{ceph-cluster-net}) et deux pour le réseau public (\texttt{ceph-public-net}), conformément au tableau~\ref{tab:fw-security-groups} du chapitre IV.\\
...
\par Les figures~\ref{fig:ann-fw-vm-haproxy} et~\ref{fig:ann-fw-vm-flask-pg} présentent les règles de pare-feu appliquées aux VMs applicatives après révision, conformément au tableau~\ref{tab:fw-vm-rules} du chapitre IV. On y observe notamment les règles ICMP ajoutées sur les VMs Flask et PostgreSQL, absentes de la configuration initiale.\\
