...
\begin{itemize}
    ...
    \item \textbf{Courbe d'apprentissage} : la maîtrise de Kubernetes est un prérequis pour administrer efficacement Harvester~\cite{ref4}.
...
\par Proxmox VE intègre LXC nativement et propose des templates de distributions populaires (Debian, Ubuntu, Alpine). Dans notre infrastructure, LXC est retenu pour tous les services qui ne nécessitent pas de noyau spécifique : monitoring (InfluxDB, Prometheus, Grafana, Loki), NTP (Chrony), TFTP/PXE, GitLab CE, Rundeck et OpenLDAP. Cette approche minimise la consommation de ressources par rapport à des VMs complètes pour des services standards.\\
...
\end{tabular}
\end{center}
\noindent \textbf{Tableau 2-3 : Comparaison VM KVM et conteneur LXC}\\
...
\par Dans un cluster de virtualisation, les VMs sont placées sur les nœuds physiques lors de leur création. Sans mécanisme d'équilibrage actif, ce placement reste statique pendant toute la durée de vie des VMs. Au fil du temps, les workloads évoluent et la charge se répartit inégalement : certains nœuds peuvent être saturés à plus de 90\,\% de CPU tandis que d'autres restent utilisés à moins de 20\,\%. Ce déséquilibre dégrade les performances des VMs sur les nœuds chargés et gaspille les ressources disponibles sur les nœuds sous-chargés.

\subsection{Le DRS de VMware : Référence Industrielle}

\par VMware vSphere propose le \textbf{DRS} (\textit{Distributed Resource Scheduler}), mécanisme d'équilibrage automatique qui analyse en continu les métriques de charge de chaque hôte et déclenche des migrations à chaud (vMotion) pour redistribuer les VMs. Le DRS calcule un score de déséquilibre pour le cluster et ne déclenche une migration que si le gain attendu dépasse un seuil configurable, afin d'éviter des migrations inutiles dans un cluster globalement sous-chargé.

\par Proxmox VE ne dispose pas d'un tel mécanisme natif. Notre projet développe un service équivalent, nommé CLBS, s'appuyant sur l'API REST Proxmox et le stockage partagé Ceph.
...
\par La live migration est le mécanisme d'exécution de toute décision d'équilibrage. Elle n'est applicable que si le stockage est partagé entre tous les nœuds (Ceph RBD dans notre cas), condition qui rend le disque de la VM accessible depuis le nœud de destination sans déplacement préalable des données~\cite{ref14}.
...
\par HAProxy (\textit{High Availability Proxy}) est un proxy TCP/HTTP open source conçu pour la haute disponibilité et l'équilibrage de charge applicatif~\cite{ref26}. Il agit comme point d'entrée unique pour un ensemble de serveurs backends, distribue les requêtes selon différents algorithmes (round-robin, least connections, source IP hash) et surveille la disponibilité des backends via des health checks périodiques. En cas d'indisponibilité d'un backend, HAProxy redirige automatiquement le trafic vers les instances opérationnelles sans interruption pour l'utilisateur.

\par Dans notre infrastructure, HAProxy constitue le point d'entrée de l'application de démonstration Sona-Web et distribue les requêtes HTTP entre deux instances Flask.

\subsection{Stack de Supervision : InfluxDB, Prometheus et Grafana}

\par La supervision d'une infrastructure hyperconvergée nécessite des outils capables de collecter, stocker et visualiser des métriques en temps réel sur l'ensemble des composants~\cite{ref25}. Trois outils complémentaires constituent la stack de monitoring de notre infrastructure.\\

\par \textbf{InfluxDB} est une base de données orientée séries temporelles (\textit{time series database}), optimisée pour le stockage de données horodatées à haute fréquence d'écriture. Elle reçoit les métriques Proxmox (CPU, RAM, réseau, stockage des nœuds et des VMs) via le mécanisme natif \textit{Metric Server}, sans agent supplémentaire. Cet historique est utilisé à la fois pour la visualisation dans Grafana et pour les décisions de migration du service CLBS.\\

\par \textbf{Prometheus} est un système de collecte de métriques par scraping HTTP, organisé autour d'un modèle de données par labels. Il collecte les métriques détaillées du cluster Ceph via l'endpoint exposé par le Ceph Manager (port 9283), ainsi que les métriques applicatives du service CLBS.\\

\par \textbf{Grafana} est un outil de visualisation connecté à InfluxDB et Prometheus comme sources de données. Il permet de construire des tableaux de bord unifiés couvrant l'état des nœuds, du cluster Ceph et du service CLBS, et d'y associer des règles d'alerte acheminées via l'Alertmanager intégré.\\

\subsection{Ansible : Automatisation d'Infrastructure}

\par Ansible est un outil d'automatisation d'infrastructure (\textit{Infrastructure as Code}) qui exécute des playbooks YAML via SSH, sans agent logiciel sur les machines cibles. Un playbook décrit l'état cible d'une machine (paquets installés, services configurés, fichiers déployés) et Ansible l'applique de manière \textbf{idempotente} : si la configuration est déjà conforme à l'état cible, aucune action n'est effectuée.

\par Chaque playbook est organisé en tâches séquentielles exécutées sur un inventaire de machines défini en YAML. La connexion SSH et les privilèges d'administration (\texttt{sudo}) sont les seuls prérequis sur les machines cibles. Dans notre infrastructure, Ansible automatise trois catégories d'opérations : l'intégration de nouveaux nœuds physiques dans le cluster, le provisionnement de VMs et de conteneurs LXC, et la promotion d'environnements de développement vers la production.
...
\par Ce mécanisme est intégré au pipeline d'automatisation pour l'installation initiale de Proxmox VE sur les serveurs physiques, sans aucune intervention manuelle sur les serveurs.
...
\begin{itemize}
    ...
    \item \textbf{Disques par n\oe{}ud} : 5 disques SAS (2 × 300 GB + 3 × 600 GB)
...
\par Ce chapitre a posé les bases théoriques du projet. Nous avons défini le concept d'hyperconvergence, comparé les principales solutions open source et justifié le choix de la stack Proxmox VE + Ceph + Open vSwitch. Nous avons présenté la pile de virtualisation KVM/QEMU, les conteneurs LXC, les mécanismes Corosync et HA, ainsi que l'architecture VxLAN gérée par Open vSwitch. Nous avons également établi les fondements théoriques de l'équilibrage dynamique de charge (algorithme CSLB, concept DRS) et présenté les outils complémentaires retenus : HAProxy, la stack de monitoring InfluxDB/Prometheus/Grafana, Ansible et PXE. Enfin, nous avons décrit le modèle de déploiement physique distribué sur trois serveurs Dell PowerEdge R720. Le chapitre suivant présente la conception détaillée de l'architecture proposée.\\
